 ...
 */
 ...
 public class RestClient extends DB {
   ...
   private int conTimeout = 1000000;
   private int readTimeout = 1000000;
   private int execTimeout = 1000000;
   ...
   private void setupClient() {
     ...
     clientBuilder.setConnectionReuseStrategy(new NoConnectionReuseStrategy());
   ...
   public Status insert(String table, String endpoint, Map<String, ByteIterator> values) {
     ...
     try {
       // System.out.println(urlPrefix + endpoint);
   ...
   private int httpExecute(HttpEntityEnclosingRequestBase request, String data) throws IOException {
     ...
     InputStreamEntity reqEntity = new InputStreamEntity(new ByteArrayInputStream(data.getBytes()), data.length(), 
     ...
     CloseableHttpResponse response = client.execute(request);
     // System.out.print("After response:");
     responseCode = response.getStatusLine().getStatusCode();
   ...
   private int httpDelete(String endpoint) throws IOException {
     ...
     // System.out.println("before second delete");
 
     // HttpDelete newrequest = new HttpDelete(endpoint);
     // for (int i = 0; i < headers.length; i = i + 2) {
     //   newrequest.setHeader(headers[i], headers[i + 1]);
     // }
     // response = client.execute(newrequest);
     // int newresponseCode = response.getStatusLine().getStatusCode();
 
     // System.out.println(newresponseCode);
     // // timer.interrupt();
     // response.close();
     // client.close();
     // newrequest.releaseConnection();
